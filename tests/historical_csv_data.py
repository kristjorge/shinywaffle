...
nvidia_bars = BarProvider('D:/PythonProjects/shiny-waffle/shiny-waffle/data/yahoo_finance/NVDA_1D.csv', '1d', '%Y-%m-%d')
nvidia.set_bars(nvidia_bars)
nvidia.add_strategy(random_signal_strat)

# Oracle stocks
oracle = Stock("Oracle", "ORCL", "USD")
oracle_bars = BarProvider('D:/PythonProjects//shiny-waffle/shiny-waffle/data/yahoo_finance/ORCL_1D.csv', '1d', '%Y-%m-%d')
oracle.set_bars(oracle_bars)
oracle.add_strategy(random_signal_strat)

# IBM stocks
ibm = Stock("IBM", "IBM", "USD")
ibm_bars = BarProvider('D:/PythonProjects//shiny-waffle/shiny-waffle/data/yahoo_finance/IBM_1D.csv', '1d', '%Y-%m-%d')
